...
# Override the install location with KVN_BIN_DIR=/some/dir — useful when you have a
# second clone and don't want it to steal the launcher from your primary one.
BIN_DIR="${KVN_BIN_DIR:-$HOME/.local/bin}"
LAUNCHER="$BIN_DIR/kvn"
mkdir -p "$BIN_DIR"

# Say so when we are repointing an existing launcher: silently stealing `kvn` from
# another clone is a confusing way to spend an afternoon.
if [ -e "$LAUNCHER" ] && ! grep -q "$REPO_ROOT/.venv" "$LAUNCHER" 2>/dev/null; then
  echo "==> NOTE: $LAUNCHER already exists and points at a different clone."
  echo "    Repointing it to $REPO_ROOT. Set KVN_BIN_DIR to install elsewhere instead."
fi
