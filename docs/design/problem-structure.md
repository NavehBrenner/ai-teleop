...
Clamp bounds (from the scope's safety envelope): `Δposition ≤ 3 cm/step` (±2 cm before the clamp recalibration), `Δorientation ≤ 10°/step`, `Δgrip ≤ 5 N/step`. Setting `Δ = 0` recovers the unassisted operator exactly — that is the "human-only" baseline for free.
