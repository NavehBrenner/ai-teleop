...
| `--assist-scale F` | `1.0` | Multiply the correction Δ before it reaches the command. `0` runs the policy in full — checkpoint load, forward pass, wrist capture — and discards its output, so it is **not** the same as `--policy noassist`, which skips all of that. That difference is what makes the two arms of a [blinded human trial](../results/human-trial-protocol.md) indistinguishable from outside the process. |
| `--no-cam-window` | off | Hide the live stereo camera + 3D-skeleton window (`--input vision`; shown by default). |
| `--gain G` | `1.0` | Hand-to-EE motion gain (`--input vision`), on top of the 1.5× workspace calibration. Higher follows the hand more aggressively and tiles the workspace with fewer clutches; lower is steadier for fine insertion. Rig-dependent — tune in free play (`--max-steps 0`) before recording. |
| `--record-hand PATH` | off | Record the stereo camera + 3D-skeleton window to a video (`--input vision`, window on). The **operator** half of demo footage; render the robot half offline from the episode with `scripts/dev/render_trajectory.py`, which keeps MuJoCo rendering out of the live loop. |
...
| `--expert-brake-gain G` | script default | Expert approach-speed brake: allowed command lead is `G·distance + floor`; 0 disables. |
| `--expert-brake-lead-floor M` | script default | Minimum allowed command lead (m) for the brake. |
| `--joint-damping KD` | script default | Controller joint-space kd (deployment config by default). |
